...
1. **The estimate of $F_{ST}$ by the Weir and Cockerham metric can sometimes generate negative values and "NA". What does that mean? How can this interfere with the results?**
   * *Answer*: degut a monomorphic sites, missing data..
2. **The $F_{ST}$ values observed between pairs of populations for the SNP rs3827760 (position 109,513,601) fall within which distribution quantiles of $F_{ST}$ values for the studied chromosome? Can they be considered outliers?**
   * *Answer*: si, en el cas de comparar europa i est d'asia es quan es veu mes clar pq punt es separa
   mes amunt i es l'unic que queda per sobre del llindar. En els altres casos hi ha mes snps que sobrepassen ell llindar especialment quan hi ha africa pq porta mes anys existint hi ha acumulat mes variants
...
5. **What does the PBS analysis reveal? What is the difference between PBS and $F_{ST}$ analysis?**
   * *Answer*: amb fst nomes veiem si hi ha diferences entre dues pobs, pero no sabem quina s'ha accelerat i per tant és susceptible d'haver estat sota seleccio positiva. 
...
```R
# 
setwd("C:/Users/Marta/Documents/embo_popgen_2026/Tabita_Hunemeier/Practical_Session_Selection_EMBO")

# =============================================================================
# FST and PBS Analysis – EDAR Gene (Chromosome 2)
...
afr_eas <- read.table("input/Part_1_HumanDiversity/AFR_EAS.weir.fst",
                      header = TRUE, stringsAsFactors = FALSE)
afr_eur <- read.table("input/Part_1_HumanDiversity/AFR_EUR.weir.fst",
                      header = TRUE, stringsAsFactors = FALSE)
eas_eur <- read.table("input/Part_1_HumanDiversity/EAS_EUR.weir.fst",
                      header = TRUE, stringsAsFactors = FALSE)

cat("Dimensions:\n")
cat("  AFR-EAS:", nrow(afr_eas), "SNPs\n")
cat("  AFR-EUR:", nrow(afr_eur), "SNPs\n")
cat("  EAS-EUR:", nrow(eas_eur), "SNPs\n\n")

# -----------------------------------------------------------------------------
# 2. FILTER DUPLICATE POSITIONS AND EXCLUDE NA VALUES
# -----------------------------------------------------------------------------
...
clean_fst <- function(df, label) {
  ...
  cat(sprintf("  %s after cleaning: %d SNPs\n", label, nrow(df)))
  return(df)
}

cat("Cleaning datasets...\n")
afr_eas <- clean_fst(afr_eas, "AFR-EAS")
afr_eur <- clean_fst(afr_eur, "AFR-EUR")
eas_eur <- clean_fst(eas_eur, "EAS-EUR")
cat("\n")

# -----------------------------------------------------------------------------
# 3. ALIGN DATASETS BY OVERLAPPING POSITIONS (3-way intersection)
# -----------------------------------------------------------------------------

common_pos <- Reduce(intersect, list(afr_eas$POS, afr_eur$POS, eas_eur$POS))
cat(sprintf("SNPs shared across all three datasets: %d\n\n", length(common_pos)))

afr_eas_shared <- afr_eas[afr_eas$POS %in% common_pos, ]
afr_eur_shared <- afr_eur[afr_eur$POS %in% common_pos, ]
eas_eur_shared <- eas_eur[eas_eur$POS %in% common_pos, ]

# Sort by position
afr_eas_shared <- afr_eas_shared[order(afr_eas_shared$POS), ]
afr_eur_shared <- afr_eur_shared[order(afr_eur_shared$POS), ]
eas_eur_shared <- eas_eur_shared[order(eas_eur_shared$POS), ]

# Build merged data frame
fst_df <- data.frame(
  CHROM   = afr_eas_shared$CHROM,
  POS     = afr_eas_shared$POS,
  FST_AFR_EAS = afr_eas_shared$WEIR_AND_COCKERHAM_FST,
  FST_AFR_EUR = afr_eur_shared$WEIR_AND_COCKERHAM_FST,
  FST_EAS_EUR = eas_eur_shared$WEIR_AND_COCKERHAM_FST
)

# -----------------------------------------------------------------------------
# 4. SET NEGATIVE FST VALUES TO ZERO
# -----------------------------------------------------------------------------
# Negative FST values arise from sampling variance when true differentiation
# is near zero; biologically they are interpreted as 0 (no differentiation).

fst_df$FST_AFR_EAS <- pmax(fst_df$FST_AFR_EAS, 0)
fst_df$FST_AFR_EUR <- pmax(fst_df$FST_AFR_EUR, 0)
fst_df$FST_EAS_EUR <- pmax(fst_df$FST_EAS_EUR, 0)

cat("Negative FST values set to 0.\n\n")

# -----------------------------------------------------------------------------
# 5. CHECK FST VALUES AT rs3827760 (position 109,513,601)
# -----------------------------------------------------------------------------

TARGET_POS <- 109513601

snp_row <- fst_df[fst_df$POS == TARGET_POS, ]

if (nrow(snp_row) == 0) {
  cat("WARNING: rs3827760 (pos", TARGET_POS, ") not found in the shared dataset.\n\n")
} else {
  cat("=== FST values at rs3827760 (chr2:", TARGET_POS, ") ===\n")
  cat(sprintf("  AFR vs EAS : %.5f\n", snp_row$FST_AFR_EAS))
  cat(sprintf("  AFR vs EUR : %.5f\n", snp_row$FST_AFR_EUR))
  cat(sprintf("  EAS vs EUR : %.5f\n\n", snp_row$FST_EAS_EUR))
}

# -----------------------------------------------------------------------------
# 6. DISTRIBUTION QUANTILES – IS rs3827760 AN OUTLIER?
# -----------------------------------------------------------------------------

probs <- c(0.50, 0.75, 0.90, 0.95, 0.99, 0.999, 1.00)

cat("=== Quantile distributions ===\n")
for (col in c("FST_AFR_EAS", "FST_AFR_EUR", "FST_EAS_EUR")) {
  q <- quantile(fst_df[[col]], probs = probs, na.rm = TRUE)
  cat(sprintf("\n%s:\n", col))
  print(round(q, 5))

  if (nrow(snp_row) > 0) {
    val  <- snp_row[[col]]
    pct  <- mean(fst_df[[col]] <= val, na.rm = TRUE) * 100
    cat(sprintf("  → rs3827760 FST = %.5f  |  percentile = %.2f%%\n", val, pct))
  }
}
...
# 7a. PBS CALCULATION
# -----------------------------------------------------------------------------
# PBS (Population Branch Statistics) for EAS relative to AFR and EUR:
#
#   T_XY = -log(1 - FST_XY)          (branch-length transformation)
#   PBS_EAS = (T_AFR_EAS + T_EAS_EUR - T_AFR_EUR) / 2
#
# A high PBS_EAS means the EAS lineage has experienced elevated divergence
# relative to AFR and EUR – a hallmark of positive selection in EAS.

fst_df <- fst_df %>%
  mutate(
    T_AFR_EAS = -log(1 - FST_AFR_EAS),
    T_AFR_EUR = -log(1 - FST_AFR_EUR),
    T_EAS_EUR = -log(1 - FST_EAS_EUR),
    PBS_EAS   = (T_AFR_EAS + T_EAS_EUR - T_AFR_EUR) / 2
  )

# Clamp negative PBS to 0 (can occur due to noise)
fst_df$PBS_EAS <- pmax(fst_df$PBS_EAS, 0)

if (nrow(snp_row) > 0) {
  snp_pbs <- fst_df[fst_df$POS == TARGET_POS, "PBS_EAS"]
  pbs_pct <- mean(fst_df$PBS_EAS <= snp_pbs, na.rm = TRUE) * 100
  cat(sprintf("PBS_EAS at rs3827760: %.5f  |  percentile = %.2f%%\n\n", snp_pbs, pbs_pct))
}

# -----------------------------------------------------------------------------
# 7b. PLOT: Pairwise FST in a 10 kb window around rs3827760
# -----------------------------------------------------------------------------

WINDOW   <- 10000   # ±10 kb
pos_min  <- TARGET_POS - WINDOW
pos_max  <- TARGET_POS + WINDOW

window_df <- fst_df %>%
  filter(POS >= pos_min, POS <= pos_max) %>%
  pivot_longer(cols = c(FST_AFR_EAS, FST_AFR_EUR, FST_EAS_EUR),
               names_to  = "Comparison",
               values_to = "FST") %>%
...
p_fst <- ggplot(window_df, aes(x = POS, y = FST, colour = Comparison)) +
  geom_point(size = 1.5, alpha = 0.7) +
  geom_line(alpha = 0.4) +
  geom_vline(xintercept = TARGET_POS, linetype = "dashed",
             colour = "black", linewidth = 0.8) +
  annotate("text", x = TARGET_POS + 200, y = max(window_df$FST, na.rm = TRUE) * 0.97,
           label = "rs3827760\n(EDAR Val370Ala)",
  ...
  scale_colour_manual(values = c("AFR vs EAS" = "#E64B35",
                                 ...
                                 "EAS vs EUR" = "#00A087")) +
  labs(
    title    = expression(paste("Pairwise ", F[ST], " around EDAR (rs3827760, chr2:109,513,601)")),
    subtitle = "±10 kb window | Weir & Cockerham estimator | 1000 Genomes Phase III",
    x        = "Chromosomal position (bp)",
    y        = expression(F[ST]),
    colour   = "Population pair"
  ) +
...
ggsave("fst_EDAR_window.png", plot = p_fst, width = 10, height = 5, dpi = 300)
cat("Plot saved: fst_EDAR_window.png\n")

# -----------------------------------------------------------------------------
# 7c. PLOT: PBS in the same window
# -----------------------------------------------------------------------------

pbs_window <- fst_df %>%
  filter(POS >= pos_min, POS <= pos_max)

p_pbs <- ggplot(pbs_window, aes(x = POS, y = PBS_EAS)) +
  geom_point(colour = "#7E6148", size = 1.8, alpha = 0.8) +
  geom_line(colour  = "#7E6148", alpha = 0.4) +
  geom_vline(xintercept = TARGET_POS, linetype = "dashed",
             colour = "black", linewidth = 0.8) +
  annotate("text", x = TARGET_POS + 200,
           y = max(pbs_window$PBS_EAS, na.rm = TRUE) * 0.95,
           label = "rs3827760\n(EDAR Val370Ala)",
           hjust = 0, size = 3, colour = "black") +
  labs(
    title    = "PBS (East Asian branch) around EDAR",
    subtitle = "±10 kb window | PBS = (T_AFR-EAS + T_EAS-EUR − T_AFR-EUR) / 2",
    x        = "Chromosomal position (bp)",
    y        = "PBS (EAS)"
  ) +
  theme_bw(base_size = 12) +
...
ggsave("pbs_EAS_EDAR_window.png", plot = p_pbs, width = 10, height = 5, dpi = 300)
cat("Plot saved: pbs_EAS_EDAR_window.png\n\n")
print(p_fst)       # FST window plot
print(p_pbs)       # PBS window plot
print(p_dist)      # Genome-wide distribution

# -----------------------------------------------------------------------------
# 8. GENOME-WIDE FST DISTRIBUTION PLOT (optional – full chr2)
# -----------------------------------------------------------------------------
...
p_dist <- fst_df %>%
  ...
  ggplot(aes(x = FST, fill = Comparison)) +
  geom_histogram(bins = 80, alpha = 0.6, position = "identity") +
  geom_vline(data = snp_row %>%
               pivot_longer(cols = c(FST_AFR_EAS, FST_AFR_EUR, FST_EAS_EUR),
               ...
               mutate(Comparison = recode(Comparison,
                 ...
                 FST_AFR_EUR = "AFR vs EUR",
                 FST_EAS_EUR = "EAS vs EUR")),
             aes(xintercept = FST, colour = Comparison),
             linetype = "dashed", linewidth = 0.9, show.legend = FALSE) +
  scale_fill_manual(values  = c("AFR vs EAS" = "#E64B35",
                                "AFR vs EUR" = "#4DBBD5",
                                "EAS vs EUR" = "#00A087")) +
  scale_colour_manual(values = c("AFR vs EAS" = "#E64B35",
                                 "AFR vs EUR" = "#4DBBD5",
                                 "EAS vs EUR" = "#00A087")) +
  labs(
    title    = expression(paste("Chromosome 2 genome-wide ", F[ST], " distribution")),
    subtitle = "Dashed lines indicate FST at rs3827760 (EDAR Val370Ala)",
    x        = expression(F[ST]),
    y        = "Count",
...
ggsave("fst_distribution_chr2.png", plot = p_dist, width = 10, height = 5, dpi = 300)
cat("Plot saved: fst_distribution_chr2.png\n")
```
hem mirat distribucio snps i sembla que si que snps son informatius que hi ha
diferencies
---
...
2. **What is the profile of ancestral and derived haplotypes of the rs3827760 SNP in AFR and EAS?**
   * *Answer*: el derivat te mes ehh que l'ancestral
...
Furcation trees ens dona informacio similar a tajima, de si s'allunya molt de la neutralitat. Cada bifurcació és un haplotip diferent. 
---
...
```
iHS (integrated haplotype score), fer-ho per windows ens permet saber la regio sota seleccio. un cop tenim la regio llavors podem corre-ho snp per snp a veure quin es el responsable.
...
nam_eas <- read.table("input/Part_1_HumanDiversity/Chr2_NAM_EAS.weir.fst",
                      header = TRUE, stringsAsFactors = FALSE)
nam_eur <- read.table("input/Part_1_HumanDiversity/Chr2_NAM_EUR.weir.fst",
                      header = TRUE, stringsAsFactors = FALSE)
eas_eur <- read.table("input/Part_1_HumanDiversity/Chr2_EUR_EAS.weir.fst",
                      header = TRUE, stringsAsFactors = FALSE)
# 
# -----------------------------------------------------------------------------
# 7a. PBS CALCULATION
...
1. **What pattern do you observe along the first principal component (PC1)?**
   * *Answer*: els gossos petits formen un cluster
2. **Why does PC1 capture body size differences in this particular dataset?**
   * *Answer*: pq s'ha triat per les practiques pq quedi maco
...
1. **If you ran PCAdapt without enabling LD clumping, a single massive peak at ~61 Mb would dominate the plot, hiding other regions. What is the effect of LD clumping on outlier detection and why is it necessary here?**
   * *Answer*: ld clumping treu redundant genetic markers que estan lligats. 
